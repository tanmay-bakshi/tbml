...
- The masked spans are **not removed** and tokens are **not replaced** with a mask token. Instead,
  masking is enforced via the attention mask:
...
- For each sample, the **global center** is the mean of the global view embeddings.
