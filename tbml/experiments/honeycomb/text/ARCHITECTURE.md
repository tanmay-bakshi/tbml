...
- TJepa-style training uses masked **global** and **local** views per sample.
  - One global view is always a **sample-level** view with no masking.
  - If global masking is enabled, additional global views are masked.
  - When `global_mask_max == 0`, exactly one global view is required. When global masking is
    enabled, at least two global views are required (one sample view + masked globals).
...
- **Transformer blocks**
  ...
  - Encoder and predictor causality are configured independently via
    `--encoder-causal-attention` and `--predictor-causal-attention`.
  - Attention respects the provided attention masks (pads and masked spans).
...
- **Predictor**
  - A separate transformer stack that operates on token‑level representations from the encoder
    output head (post‑head). Causality follows the `--predictor-causal-attention` setting.
...
- Reconstruction uses only the predictor outputs (masked globals + locals); the sample‑level
  global view is excluded from the reconstruction loss and is only used to form the SWA
  global center.
