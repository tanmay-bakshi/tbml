...
from tbml.nn import DropPath, Linear, PoPESelfAttention, RMSNorm, RoPESelfAttention, SwiGLUFeedForward
...
def _parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Train a transformer policy head on frozen token embeddings."
    )
...
class PolicyTransformerBlock(eqx.Module):
    """Transformer block for policy sequences."""
...
class PolicyTransformer(eqx.Module):
    """Transformer policy network that predicts tokens from attention outputs."""
    ...
    def __init__(
        ...
        *,
        ...
        block_keys = keys[: config.n_layers]
        head_key = keys[-2]
        norm_key = keys[-1]
...
def main() -> None:
    """Run the transformer policy training loop."""
    args = _parse_args()
    ...
    policy_attn_type = args.policy_attn_type
    if policy_attn_type is None:
        policy_attn_type = base_model.config.attn_type
    ...
    policy_config = PolicyTransformerConfig(
        ...
        attn_type=str(policy_attn_type),
    ...
    def grad_step(
        ...
        def _loss_fn(
            ...
            logits = policy_inner(reps, attention_mask=attention_mask, train=True, key=tokens_key)
