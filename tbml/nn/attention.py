...
def _rope_frequencies(head_dim: int, *, base: float) -> jnp.ndarray:
    """Compute RoPE angular frequencies.

    :param head_dim: Per-head dimension.
    :param base: Base for frequency computation.
    :returns: Frequency vector of shape (head_dim // 2,).
    """
    if head_dim <= 0:
        raise ValueError("head_dim must be > 0")
    if head_dim % 2 != 0:
...
def _apply_rope(x: Array, angles: Array) -> Array:
    ...
    :param x: Tensor of shape (B, H, T, D).
    :param angles: Rotation angles of shape (T, D/2).
    :returns: Rotated tensor of shape (B, H, T, D).
    """
    if x.ndim != 4:
        raise ValueError("x must have shape (B, H, T, D)")
    if angles.ndim != 2:
        raise ValueError("angles must have shape (T, D/2)")
    if x.shape[2] != angles.shape[0]:
        raise ValueError("angles must match sequence length")
    if x.shape[3] % 2 != 0:
        raise ValueError("head dimension must be even for RoPE")

    bsz, n_heads, seqlen, head_dim = x.shape
    half_dim = head_dim // 2
    if angles.shape[1] != half_dim:
        raise ValueError("angles must match head dimension")

    cos = jnp.cos(angles)[None, None, :, :]
    sin = jnp.sin(angles)[None, None, :, :]
    x = x.reshape((bsz, n_heads, seqlen, half_dim, 2))
    x1 = x[..., 0]
    x2 = x[..., 1]
    rot1 = x1 * cos - x2 * sin
    rot2 = x1 * sin + x2 * cos
    rotated = jnp.stack([rot1, rot2], axis=-1)
    return rotated.reshape((bsz, n_heads, seqlen, head_dim))
...
class RoPESelfAttention(eqx.Module):
    ...
    :ivar resid_dropout: Residual dropout probability.
    :ivar is_causal: Whether to apply a causal attention mask.
    :ivar base: Base for RoPE frequency schedule.
    :ivar dtype: Compute dtype.
    :ivar param_dtype: Parameter dtype.
    :ivar q_proj: Query projection.
    :ivar k_proj: Key projection.
    ...
    def __init__(
        ...
        *,
        ...
        """Initialize RoPE attention parameters.
        ...
        :param resid_dropout: Residual dropout probability.
        :param is_causal: Whether to apply a causal attention mask.
        :param base: Base for the RoPE frequency schedule.
        :param dtype: Compute dtype.
        :param param_dtype: Parameter dtype.
        :param qkv_kernel_init: Initializer for Q/K/V projections.
        :param o_kernel_init: Initializer for output projection.
    ...
    def __call__(
        ...
        *,
        ...
        positions = jnp.arange(seqlen, dtype=jnp.float32)
        freqs = _rope_frequencies(head_dim, base=self.base)
        angles = positions[:, None] * freqs[None, :]

        qf = _apply_rope(q.astype(jnp.float32), angles)
        kf = _apply_rope(k.astype(jnp.float32), angles)
        vf = v.astype(v.dtype)
